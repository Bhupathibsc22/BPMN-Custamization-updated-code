...
export class DiagramComponent implements AfterContentInit, OnDestroy {
  ...
  ngAfterContentInit(): void {
    ...
    DiagramComponent.instance = this.bpmnJS;
    this.bpmnJS.get('palette').registerProvider(11,{
      createPalleteEntries:()=>{
        return {
        'custom-create-task': {
          group: '',
    ...
    function createTask(event) {
      const shape =  DiagramComponent.instance.get('elementFactory').createShape({ type:"bpmn:Task"});
      shape.businessObject.name = "Test task";
      event.dataTransfer.setData('text/plain', 'custom-task');
      event.dataTransfer.setData("application/bpmn", JSON.stringify(shape));
      // const shape = this.bpmnJS..createShape({ type: 'bpmn:Task' });
